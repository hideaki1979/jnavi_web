/**
 * スモークテスト用のスタブバックエンドAPI。**既定では使わない。**
 *
 * スモークテストは実バックエンド（nodedeploytest）に対して実行する。
 * このスタブは `E2E_USE_MOCK_API=1` を付けたときだけ起動する退避経路で、
 * バックエンドと PostgreSQL を用意できない状況（CI など）向けのもの。
 *
 * ```bash
 ...
 *
 * 実バックエンドに追随しているかは自動では検証されない。
 * レスポンス形が変わったときにここが古いままだと、
 * スタブ経由のテストだけが通り続けることになる点に注意。
 *
