...
J.Navi は、二郎系ラーメン愛好家のための専門的な店舗情報共有プラットフォームです。Next.js 16 の App Router と React 19 を使用し、Material UI と Tailwind CSS でスタイリングされています。Google Maps との連携によるインタラクティブな店舗マップ、画像管理機能、そして初心者でも安心して注文できるコールシミュレーション機能を提供し、二郎系ラーメンの文化を広めることを目指しています。
...
| **フレームワーク**     | <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/nextjs/nextjs-original.svg" width="20" height="20" alt="Nextjs Logo"/> Next.js                | 16.3.3     | React ベースのフルスタックフレーム     |
...
| **UI フレームワーク**  | <img src="https://mui.com/static/logo.svg" width="20" height="20" alt="MUI Logo"/> Material UI                                                          | 7.1.0      | モダンな UI コンポーネント             |
...
| **HTTP クライアント**  | <img src="https://axios-http.com/assets/favicon.ico" width="20" height="20" alt="Axios Logo"/> Axios                                                      | 1.19.0     | API 通信                               |
| **フォーム管理**       | React Hook Form                                                                                                                          | 7.56.4     | フォームバリデーション・状態管理       |
| **バリデーション**     | Zod                                                                                                                                      | 3.25.7     | スキーマバリデーション                 |
| **認証・ストレージ**   | <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/firebase/firebase-plain.svg" width="20" height="20" alt="Firebase Logo"/> Firebase              | 12.17.1    | 認証・ファイルストレージ               |
...
認証まわりの分岐を、外部サービス無しに確定的に検証するテストです。
現在の対象は4ファイルで、テストは対象ファイルの隣に `*.test.ts` として置いています。

| 対象 | 主に見ているもの |
| --- | --- |
| `src/app/api/auth/session/route.ts` | `auth_time` の境界（直近5分以内のサインインのみクッキーへ引き換える）、失効に失敗したらクッキーを消さないこと、発行と削除でクッキーの属性が一致すること |
...
| `src/utils/redirectPath.ts` | `redirect_to` が自サイト内のパスに限られること（オープンリダイレクト対策）、通すべき内部パスまで潰していないこと |
...
現状は DOM に触らないコードだけなので `node` 環境で実行します。
...
主要ページが開いて、中身が描かれていて、コンソールにエラーが出ないことを
機械的に確認するテストです。
...
**店舗が1件以上登録されていて、そのうち1件以上が座標を持っている必要があります。**
`e2e/global-setup.ts` が実行前に次の2つを取得します。

- `GET /stores` の先頭の店舗ID … 事前コール・着丼前コールのルートを開くのに使う
- `GET /maps` の先頭の店舗の座標 … マップを開くときの現在地に使う

どちらも取得できない場合は決め打ちの値で続行せず、理由を示して停止します。
決め打ちで続行すると、そのIDが実在しない環境では該当ルートが
`[ActionError] ... status:404` で落ち、店舗が1件も地図に出せない環境では
ピンの検証が落ちて、いずれも環境の問題がアプリの退行に見えてしまうためです。
...
| 着地先のパス名が想定と違う | 失敗。意図しないリダイレクトを見逃さないため |
| ルートごとの目印が見つからない | 失敗。中身が空のページを見逃さないため |
...
各ルートには「これが見えていれば描画は生きている」と言える目印を
1つずつ定義しています（`e2e/routes.ts` の `expectedContent`）。
ページが開くことしか見ていないと、データの取得経路が壊れて中身が空になった状態が
緑のまま通り抜けるためです。たとえば `GET /maps` が `{ data: [] }` を返しても、
HTTP は 200、着地先のパスも同じ、サーバーコンポーネントは例外を投げず、
コンソールにも何も出ないので、ピンが1本も無いマップを描いたまま通ってしまいます。

目印は `data-testid` を足さずに済むよう、可能な限りロールとアクセシブルネームで
指定しています（`role=button[name="ログイン"]`）。アクセシビリティ上の退行も
同時に拾えるためです。ルートを追加するときも1つ決めてください
（E2E の網羅に踏み込まないよう、1ルート1要素に留めています）。
...
├── global-setup.ts        # 実在する店舗IDと店舗座標の取得
